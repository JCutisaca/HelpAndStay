        ...
        <div className="absolute flex flex-col md:flex-row rounded-3xl border border-black shadow-2xl w-[90%] md:w-[40rem] h-[40rem] top-[5rem] z-[5] left-[50%] transform -translate-x-1/2">
            <div className="w-full h-full md:w-[50%] flex flex-col items-center justify-evenly bg-white rounded-3xl md:rounded-e-[0] md:rounded-s-3xl">
                ...
                <div className="w-[80%] flex flex-col items-center gap-4">
                    ...
                    <button onClick={() => handleSignInFacebook()} className="flex items-center justify-center md:justify-start gap-4 text-nowrap text-base font-monserrat font-semibold w-full text-white rounded-xl bg-[#1877F2] pl-4 border-[#1877F2] border-solid border-2">
                        <svg
                            aria-hidden="true"
                            width="18" height="18"
                            viewBox="0 0 18 18">
                ...
                <p className="text-base font-monserrat font-semibold text-center text-black w-[80%]">Or</p>
                <form className="w-[80%] flex flex-col items-center gap-4">
                    <input className="text-base font-monserrat w-full text-[#5196A6] border-[#D9D9D9] border-solid border-[3px] rounded-xl pl-2" placeholder="Email" type="text" />
                    <input className="text-base font-monserrat w-full text-[#5196A6] border-[#D9D9D9] border-solid border-[3px] rounded-xl pl-2" placeholder="Password" type="text" />
                    <button className="text-base font-monserrat font-semibold w-full text-[#5196A6] border border-solid border-[3px] rounded-xl hover:bg-[#F3A342] hover:text-white hover:border-[#F3A342] transition-all duration-500" type="submit">Login</button>
                </form>
                <p className="text-xs font-monserrat text-black w-[90%]">By continuing, you accept our terms of service. Please read our privacy policy.</p>
            </div>
            <div className="hidden md:flex w-[50%] rounded-r-3xl">
                <Image className="w-full h-full object-cover rounded-r-3xl" priority loading="eager" alt="" src={login} />
            </div>
        </div>
    )
}